...
class testBuilder(unittest.TestCase):
    ...
    def testInitialize(self):
        self.builder1 = self.builder1 + self.trend + self.dynamic + self.autoReg

        self.builder1.initialize(data=self.data)
        self.assertAlmostEqual(
            np.sum(
                np.abs(
                    self.builder1.model.evaluation
                    - mt.matrixAddByCol(
                        mt.matrixAddByCol(
                            self.trend.evaluation, self.dynamic.evaluation
                        ),
                        self.autoReg.evaluation,
                    )
                )
            ),
            0.0,
    ...
    def testInitializeEvaluatoin(self):
        ...
        self.assertAlmostEqual(
            np.sum(
                np.abs(
                    self.builder1.model.evaluation
                    - mt.matrixAddByCol(self.trend.evaluation, self.dynamic.evaluation)
                )
            ),
            0.0,
        )
        ...
        self.assertAlmostEqual(
            np.sum(
                np.abs(
                    ...
                    - mt.matrixAddByCol(
                        mt.matrixAddByCol(
                            self.trend.evaluation, np.array([self.features[2]])
                        ),
                        self.autoReg.evaluation,
                    )
                )
    ...
    def testInitializFromBuilder(self):
        ...
        self.builder2.initializeFromBuilder(
            exported_builder=self.builder1, data=self.data1
        )

        # make sure builder1 and builder2 are identical.
        self.assertDictEqual(
            self.builder1.staticComponents, self.builder2.staticComponents
        )
        self.assertDictEqual(
            self.builder1.dynamicComponents, self.builder2.dynamicComponents
        )
        self.assertDictEqual(
            self.builder1.automaticComponents, self.builder2.automaticComponents
        )
