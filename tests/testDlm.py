...
class testDlm(unittest.TestCase):
    ...
    def testAppend(self):
        ...
        self.assertAlmostEqual(
            np.sum(
                ...
                - np.array(self.dlm1.result.filteredObs)
            ),
            0.0,
        )

    def testAppendEmptyDLM(self):
        dlm_original = dlm(self.data[0:20])
        dlm_original + trend(degree=1, discount=1) + autoReg(degree=2, discount=1)
        dlm_original.fitForwardFilter()

        dlm_append = dlm(data=[])
        dlm_append + trend(degree=1, discount=1) + autoReg(degree=2, discount=1)
        dlm_append.append(self.data[0:20])
        dlm_append.fitForwardFilter()

        np.testing.assert_array_equal(dlm_original.getMean(), dlm_append.getMean())
    ...
    def testAppendDynamic(self):
        ...
        self.assertAlmostEqual(
            np.sum(
                np.array(dlm4.result.filteredObs) - np.array(dlm5.result.filteredObs)
            ),
            0.0,
        )
    ...
    def testOneDayAheadPredictWithoutDynamic(self):
        ...
        self.assertAlmostEqual(
            self.dlm3._predictModel.result.predictStatus, [11, 12, [-6.0 / 7]]
        )
        ...
        self.assertAlmostEqual(
            self.dlm3._predictModel.result.predictStatus, [2, 3, [3.0 / 5]]
        )

    def testOneDayAheadPredictWithDynamic(self):
        self.dlm4.fitForwardFilter()
        featureDict = {"dynamic": 2.0}
        (obs, var) = self.dlm4.predict(date=9, featureDict=featureDict)
    ...
    def testContinuePredictWithoutDynamic(self):
        ...
        self.assertAlmostEqual(
            self.dlm3._predictModel.result.predictStatus, [11, 13, [-6.0 / 7, 6.0 / 7]]
        )
    ...
    def testContinuePredictWithDynamic(self):
        ...
        (obs, var) = self.dlm4.predict(date=9, featureDict=featureDict)
        self.assertAlmostEqual(
            self.dlm4._predictModel.result.predictStatus, [9, 10, [5.0 / 6 * 2]]
        )

        featureDict = {"dynamic": [3.0]}
        (obs, var) = self.dlm4.continuePredict(featureDict=featureDict)
        self.assertAlmostEqual(
            self.dlm4._predictModel.result.predictStatus,
    ...
    def testPredictNWithDynamic(self):
        ...
        (obs, var) = self.dlm4.predictN(N=2, date=9, featureDict=featureDict)
        self.assertAlmostEqual(
            self.dlm4._predictModel.result.predictStatus,
            [9, 11, [5.0 / 6 * 2, 5.0 / 6 * 3]],
        )
    ...
    def testGetLatentState(self):
        ...
        for i in range(len(filteredTrend)):
            diff += abs(filteredTrend[i][0] - self.dlm5.result.filteredState[i][0, 0])
        ...
        for i in range(len(smoothedTrend)):
            diff += abs(smoothedTrend[i][0] - self.dlm5.result.smoothedState[i][0, 0])
        self.assertAlmostEqual(diff, 0)

    def testGetLatentCov(self):
        # for forward filter
        self.dlm5.fitForwardFilter()
        filteredTrend = self.dlm5.getLatentCov(filterType="forwardFilter", name="trend")
        diff = 0.0
        for i in range(len(filteredTrend)):
            diff += abs(filteredTrend[i][0, 0] - self.dlm5.result.filteredCov[i][0, 0])
        self.assertAlmostEqual(diff, 0)
        ...
        diff = 0.0
        for i in range(len(smoothedTrend)):
            diff += abs(smoothedTrend[i][0, 0] - self.dlm5.result.smoothedCov[i][0, 0])
        self.assertAlmostEqual(diff, 0)
    ...
    def testGetMean(self):
        ...
        diff = 0.0
        for i in range(len(filteredTrend)):
            diff += abs(filteredTrend[i] - self.dlm5.result.filteredObs[i][0, 0])
        self.assertAlmostEqual(diff, 0)
        ...
        diff = 0.0
        for i in range(len(filteredTrend)):
            diff += abs(filteredTrend[i] - self.dlm5.result.smoothedObs[i][0, 0])
        self.assertAlmostEqual(diff, 0)
    ...
    def testGetVar(self):
        ...
        diff = 0.0
        for i in range(len(filteredTrend)):
            diff += abs(filteredTrend[i] - self.dlm5.result.filteredObsVar[i][0, 0])
        self.assertAlmostEqual(diff, 0)
        ...
        diff = 0.0
        for i in range(len(filteredTrend)):
            diff += abs(filteredTrend[i] - self.dlm5.result.smoothedObsVar[i][0, 0])
        self.assertAlmostEqual(diff, 0)
    ...
    def testGetMSE(self):
        ...
        mse_expect = 0
        for i in range(20):
            mse_expect += (self.dlm1.result.predictedObs[i] - self.data[i]) ** 2
        mse_expect /= 20
