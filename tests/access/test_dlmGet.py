...
class test_dlmGet(unittest.TestCase):
    ...
    def testGetLatentState(self):
        ...
        for i in range(len(predictedTrend)):
            diff += abs(predictedTrend[i][0] - self.dlm5.result.predictedState[i][0, 0])
        self.assertAlmostEqual(diff, 0)
    ...
    def testGetLatentCov(self):
        ...
        for i in range(len(predictedTrend)):
            diff += abs(
                predictedTrend[i][0, 0] - self.dlm5.result.predictedCov[i][0, 0]
            )
        self.assertAlmostEqual(diff, 0)
    ...
    def testComponentMean(self):
        ...
        arTrend = self.dlm5._getComponentMean(
            filterType="forwardFilter", name="ar2", start=0, end=99
        )
        ...
        self.dlm5.result.smoothedSteps = [0, 99]
        arTrend = self.dlm5._getComponentMean(
            filterType="backwardSmoother", name="ar2", start=0, end=99
        )
    ...
    def testComponentVar(self):
        ...
        self.dlm5.result.filteredSteps = [0, 99]
        # for component with forward filter
        arTrend = self.dlm5._getComponentVar(
            filterType="forwardFilter", name="ar2", start=0, end=99
        )
