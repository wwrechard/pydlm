...
class test_dlmTune(unittest.TestCase):
    def setUp(self):
        ...
        (
            self.dlm6.builder
            + trend(degree=0, discount=0.9, w=1.0)
    ...
    def testComputeMSE(self):
        ...
        for i in range(7):
            if self.dlm7.data[i] is not None:
                mse_expect += (
                    self.dlm7.result.predictedObs[i] - self.dlm7.data[i]
                ) ** 2
        mse_expect /= 7
    ...
    def testSetDiscount(self):
        ...
        self.assertAlmostEqual(
            self.dlm6.builder.staticComponents["trend"].discount, 0.9
        )

        self.dlm6._setDiscounts([0.0, 0.1, 0.2], True)
        self.assertTrue(
            self.dlm6.builder.staticComponents["trend"].discount in [0.0, 0.1, 0.2]
        )
