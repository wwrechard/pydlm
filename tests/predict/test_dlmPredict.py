...
class test_dlmPredict(unittest.TestCase):
    ...
    def testOneDayAheadPredictWithoutDynamic(self):
        ...
        self.assertAlmostEqual(self.dlm3.result.predictStatus, [11, 12, [-6.0 / 7]])
        ...
        self.assertAlmostEqual(self.dlm3.result.predictStatus, [2, 3, [3.0 / 5]])
    ...
    def testOneDayAheadPredictWithDynamic(self):
        ...
        (obs, var) = self.dlm4._oneDayAheadPredict(date=9, featureDict=featureDict)
    ...
    def testContinuePredictWithoutDynamic(self):
        ...
        self.assertAlmostEqual(
            self.dlm3.result.predictStatus, [11, 13, [-6.0 / 7, 6.0 / 7]]
        )
    ...
    def testContinuePredictWithDynamic(self):
        ...
        featureDict = {"dynamic": 2.0}
        (obs, var) = self.dlm4._oneDayAheadPredict(date=9, featureDict=featureDict)
        self.assertAlmostEqual(self.dlm4.result.predictStatus, [9, 10, [5.0 / 6 * 2]])

        featureDict = {"dynamic": 3.0}
        (obs, var) = self.dlm4._continuePredict(featureDict=featureDict)
        self.assertAlmostEqual(
            self.dlm4.result.predictStatus, [9, 11, [5.0 / 6 * 2, 5.0 / 6 * 3]]
        )
