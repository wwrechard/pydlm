...
def plotInMultipleFigure(time, data, result, options):
    ...
    numOfPlots = (
        options.plotFilteredData + options.plotPredictedData + options.plotSmoothedData
    )
    size = (numOfPlots, 1)
    location = 1
...
def plotLatentState(time, coordinates, result, options, name):
    ...
    for i, dim in enumerate(coordinates):
        ...
        plt.title("Filter result for dimension " + str(i) + " in component: " + name)
...
def plotSingleState(time, dimension, result, options):
    ...
    if options.plotFilteredData:
        ...
        var = [
            abs(item[dimension, dimension]) for item in result.filteredCov[start:end]
        ]
    ...
    if options.plotPredictedData:
        ...
        data = [item[dimension, 0] for item in result.predictedState[start:end]]
        var = [
            abs(item[dimension, dimension]) for item in result.predictedCov[start:end]
        ]
    ...
    if options.plotSmoothedData:
        ...
        var = [
            abs(item[dimension, dimension]) for item in result.smoothedCov[start:end]
        ]
...
def plotComponent(time, data, result, options):
    ...
    if options.separatePlot:
        numOfPlots = (
            options.plotFilteredData
            + options.plotPredictedData
...
# =========================== plot for prediction ========================
def plotPrediction(time, data, predictedTime, predictedData, predictedVar, options):
    """
    ...
    if options.showConfidenceInterval:
        upper, lower = getInterval(predictedData, predictedVar, p=options.confidence)
...
def plotInterval(time, upper, lower, intervalType, color="black"):
    ...
    if time is None:
        if intervalType == "line":
            ...
            plt.fill_between(upper, lower, facecolor=color, alpha=ALPHA)
    else:
        if intervalType == "line":
            plt.plot(time, upper, "--", color=color)
            plt.plot(time, lower, "--", color=color)
        elif intervalType == "ribbon":
            plt.fill_between(time, upper, lower, facecolor=color, alpha=ALPHA)
