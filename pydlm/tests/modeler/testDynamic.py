...
class testDynamic(unittest.TestCase):
    ...
    def testInitialization(self):        
        ...
        self.assertAlmostEqual(np.abs(np.sum(np.matrix(self.newDynamic.features)
                                             - np.matrix(self.features))), 0)

    def testUpdate(self):
        self.newDynamic.updateEvaluation(3)
        self.assertAlmostEqual(np.abs(np.sum(np.array(self.newDynamic.evaluation)
                                             - np.array(self.features[3]))), 0)

    def testAppendNewData(self):
        self.newDynamic.appendNewData([[1, 2]])
        self.assertAlmostEqual(np.abs(np.sum(np.array(self.newDynamic.features[-1])
                                             - np.array([1, 2]))), 0)
